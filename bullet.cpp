...
Bullet::Bullet(QPoint startPos, QPoint targetPoint, int damage, Enemy *target,
               MainWindow *game, const QPixmap &sprite/* = QPixmap(":/image/attack1.png")*/)
	: m_startPos(startPos)
...
void Bullet::hitTarget()
{
    ...
    if (m_game->enemyList().indexOf(m_target) != -1)
    {
        ...
        {
            m_target->getDamage(m_damage,0);//普通子弹
        }
        else /*if(typeid(*this) == typeid(Bullet1))*/
        {
            m_target->getDamage(m_damage,1);//减速子弹
        }
    }
